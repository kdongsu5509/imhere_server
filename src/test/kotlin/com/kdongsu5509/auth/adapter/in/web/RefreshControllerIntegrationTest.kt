...
class RefreshControllerIntegrationTest : WebIntegrationTestSupport() {
    ...
    fun refreshFailWhenTokenExpired() {
        // given - 만료된 토큰은 이미 Redis에서 삭제되었거나 서명이 유효하지 않음을 시뮬레이션
        // (실제로는 Redis TTL이 지난 토큰 or 다른 서버에서 발급한 서명을 가진 토큰)
    ...
    @Test
    @DisplayName("Redis에 존재하지 않는(로그아웃된) 리프레시 토큰으로 재발급 시 401 Unauthorized와 에러를 반환하며 문서화한다")
    fun refreshFailWhenTokenNotFoundInRedis() {
        ...
        mockMvc.perform(
            ...
            .andDo(
                MockMvcRestDocumentationWrapper.document(
                    identifier = "auth-refresh-fail-token-not-in-redis",
                    snippets = arrayOf(
                        responseFields(
